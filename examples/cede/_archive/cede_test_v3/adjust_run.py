...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="transformation adjustments and execution"
    )
    parser.add_argument(
        "-c",
        "--config",
...
def factorize_sublimits(df_location, df_account):
    df_location["CondName"] = (
        df_location["ContractID"] + "--" + df_location["SublimitArea"]
    )
    df_cond = df_location[
        ["ContractID", "SublimitArea", "CondName"]
    ].drop_duplicates()
...
def main():
    ...
    df_reinsurance = pd.read_csv(
        reinsurance_file, encoding=input_file_encoding
    )
    ...
    df_location[["ConstructionCode", "OccupancyCode"]] = pad_codes(
        df_location[["ConstructionCode", "OccupancyCode"]], "0", 4
    )
    ...
    if ara_adjustment:
        ...
        df_location["LocPerilsCovered"] = df_location[
            "LocPerilsCovered"
        ].apply(lambda x: check_peril(x))
